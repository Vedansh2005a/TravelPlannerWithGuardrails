...
def _json_text_llm(text: str) -> dict[str, Any]:
    ...
    if start == -1 or end == -1 or end < start:
        raise ValueError(
            "The data from the llm is not in the format of JSON"
        )
...
"""
    ...
    try:
        ...
        allowed = bool(
            guardrails_json.get("allowed", True)
        )

        guardrail_reason = str(
            guardrails_json.get("reason", "").strip()
        )
        ...
        allowed = True
        guardrail_reason = (
            "Guardrails validation fallback allowed the request."
        )

    if not allowed:
        reason = (
            guardrail_reason
            or "TravelPlanner AI can only answer travelling questions."
        )
        ...
        return {
            ...
            "messages": [
                AIMessage(
                    content=f"Guardrails Blocked Request: {reason}"
                )
            ],
            "llm_calls": llm_call
        }
...
"""
    ...
    return {
        ...
        "messages": [
            AIMessage(
                content="Budget Generated Successfully"
            )
        ],
        "llm_calls": state.get("llm_calls", 0) + 1
    }
...
"""
    ...
    approval_request = (
        "Please review the generated draft itinerary. Approve it "
        "to create the final polished plan, or provide feedback "
        "for revision."
    )
...
def final_agent(state: TravelState):
    if state.get("approved", False):
        review_instruction = (
            "The user approved the draft. Preserve its decisions "
            "while polishing it."
        )
